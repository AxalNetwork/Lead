...
-- --------------------- unrecoverable rows → manual-review sentinel marker
-- Architect round-2/3 feedback: every corrupted name row must be
-- handled deterministically. Rows whose canonical domain column is
-- missing/malformed get a `[needs manual review] <original>` sentinel
-- prefix so operators can spot + remap them on the dashboard. The
-- sentinel never matches the type-string allowlist (it has spaces +
-- mixed case), so reruns are idempotent.
UPDATE firms
SET name = '[needs manual review] ' || name
WHERE name IS NOT NULL
  AND name NOT LIKE '[needs manual review]%'
  AND (
    lower(trim(name)) IN (
      'vc','pe','angel','accelerator','incubator','nonprofit','bootcamp',
      'network','platform','micro vc','corporate vc','fellow program',
...
UPDATE u_entities
SET display_name = '[needs manual review] ' || display_name
WHERE display_name IS NOT NULL
  AND display_name NOT LIKE '[needs manual review]%'
  AND (
    lower(trim(display_name)) IN (
      'vc','pe','angel','accelerator','incubator','nonprofit','bootcamp',
      'network','platform','micro vc','corporate vc','fellow program',
