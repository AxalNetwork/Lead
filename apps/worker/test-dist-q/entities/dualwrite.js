...
import { insertFactsBatch } from "./facts";
...
async function resolveOrCreate(env, table, legacyId, kind, init, channelLookups) {
    ...
    if (kind === "org" && init.primary_domain) {
        const r = await env.DB.prepare(`SELECT id FROM u_entities
        WHERE primary_domain = ? AND status NOT IN ('merged','soft_deleted')
        LIMIT 1`).bind(init.primary_domain).first();
        if (r?.id) {
    ...
    if (kind === "person" && init.primary_email_key) {
        ...
        LIMIT 1`).bind(init.primary_email_key).first();
        if (r?.id) {
            await setLegacyEntityId(env, table, legacyId, r.id);
            return r.id;
        }
    ...
    if (init.primary_linkedin_key) {
        ...
        LIMIT 1`).bind(init.primary_linkedin_key).first();
        if (r?.id) {
            await setLegacyEntityId(env, table, legacyId, r.id);
            return r.id;
        }
    ...
    try {
        ...
        if (!created)
            return null; // Task #9: rejected by garbage detector
        await setLegacyEntityId(env, table, legacyId, created.id);
        return created.id;
    }
    catch (e) {
        console.warn("dualwrite createEntity failed", table, legacyId, e.message);
        return null;
    }
}
function chan(env, entityId, kind, raw, source, primary = false) {
    if (!raw)
        return Promise.resolve(null);
    return upsertChannel(env, { entity_id: entityId, kind, canonical: String(raw), source, is_primary: primary });
}
export async function syncFirmToEntity(env, f, source = "firms_upsert", sourceKind = "scrape") {
    try {
        ...
        if (f.kind && /accelerator/i.test(f.kind))
            await addRole(env, entityId, "accelerator", { source });
        ...
        try {
            ...
            console.warn("inferAndAssignRoles(firm) failed", entityId, e.message);
        }
        const patches = [
        ...
        console.warn("syncFirmToEntity failed", f.id, e.message);
        return null;
    }
}
export async function syncLeadToEntity(env, l, source = "leads_repo", sourceKind = "scrape") {
    try {
        ...
        if (l.investor_kind)
            await addRole(env, entityId, "investor", { is_primary: true, source });
        if (l.category && /founder|ceo|cto/i.test(l.category))
            await addRole(env, entityId, "founder", { source });
        ...
        try {
            ...
            console.warn("inferAndAssignRoles(lead) failed", entityId, e.message);
        }
        const patches = [
        ...
        console.warn("syncLeadToEntity failed", l.id, e.message);
        return null;
    }
}
export async function syncCompanyToEntity(env, c, source = "companies") {
    try {
        ...
        if (c.stage)
            await addTag(env, { entity_id: entityId, taxonomy: "stage", slug: c.stage, source });
        await enqueueSummaryRebuild(env, entityId);
        return entityId;
    }
    catch (e) {
        console.warn("syncCompanyToEntity failed", c.id, e.message);
        return null;
    }
}
export async function syncAccountToEntity(env, a, source = "accounts") {
    try {
        ...
        if (a.industry)
            await addTag(env, { entity_id: entityId, taxonomy: "sector", slug: a.industry, source });
        await enqueueSummaryRebuild(env, entityId);
        return entityId;
    }
    catch (e) {
        console.warn("syncAccountToEntity failed", a.id, e.message);
        return null;
    }
}
export async function syncBuyerToEntity(env, b, source = "buyers") {
    try {
        ...
        if (b.is_decision_maker)
            await addRole(env, entityId, "executive", { source });
        // Link buyer → account via 'works_at' edge.
        const accountEntityId = await getLegacyEntityId(env, "accounts", b.account_id);
        if (accountEntityId) {
            await env.DB.prepare(`INSERT INTO rel_edges (id, src_entity_id, dst_entity_id, kind, source)
         VALUES (?, ?, ?, 'works_at', ?)
         ON CONFLICT(src_entity_id, dst_entity_id, kind, IFNULL(valid_from,'')) DO NOTHING`).bind(crypto.randomUUID(), entityId, accountEntityId, source).run().catch(() => undefined);
        ...
        if (b.role_slug)
            await addTag(env, { entity_id: entityId, taxonomy: "role", slug: b.role_slug, source });
        await enqueueSummaryRebuild(env, entityId);
        return entityId;
    }
    catch (e) {
        console.warn("syncBuyerToEntity failed", b.id, e.message);
        return null;
    }
}
