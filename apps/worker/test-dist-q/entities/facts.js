...
export async function insertFact(env, f) {
    ...
    const lock = await (async () => {
        try {
            return await env.DB.prepare(`SELECT 1 FROM field_overrides
          WHERE entity_id = ? AND predicate = ? AND locked = 1
            AND (unlock_after IS NULL OR unlock_after > datetime('now'))
          LIMIT 1`).bind(f.entity_id, f.predicate).first();
        }
        catch (e) {
    ...
    try {
       ...
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`).bind(id, f.entity_id, f.predicate, f.value_text ?? null, f.value_number ?? null, f.value_json != null ? JSON.stringify(f.value_json) : null, f.value_entity_id ?? null, f.source_kind, f.source ?? null, f.evidence_url ?? null, f.confidence ?? 1, now, f.valid_from ?? null, f.valid_to ?? null, hash, supersededByOverride).run();
        ...
        if (!supersededByOverride) {
            await env.DB.prepare(`UPDATE facts SET superseded_by_override = 1
          WHERE id = ?
            AND EXISTS (
        ...
        if (isRelevantPredicate(f.predicate)) {
            ...
            void triggerEntityMatchRefresh(env, f.entity_id).catch((e) => {
                console.warn("triggerEntityMatchRefresh from insertFact failed", e.message);
            });
        }
        ...
        if (/UNIQUE/i.test(msg)) {
            try {
                await env.DB.prepare("UPDATE facts SET observed_at = ? WHERE hash = ?").bind(now, hash).run();
            }
            catch (uErr) {
                console.warn("insertFact observed_at refresh failed", uErr.message);
            }
            return null;
        }
        throw e;
...
export async function loadCurrentOverrides(env, entityId) {
    const r = await env.DB.prepare(`SELECT id, predicate, value_text, value_numeric, value_json, overridden_at
       FROM field_overrides
      WHERE entity_id = ? AND locked = 1
        AND (unlock_after IS NULL OR unlock_after > datetime('now'))
      ORDER BY overridden_at DESC`).bind(entityId).all().catch(() => ({ results: [] }));
    const map = new Map();
    for (const o of r.results ?? []) {
        if (!map.has(o.predicate))
            map.set(o.predicate, o);
    }
    return map;
}
export async function getEffectiveFacts(env, entityId, opts) {
    const factWhere = opts?.includeNonCurrent ? "" : " AND is_current = 1";
    const limit = opts?.limit ?? 500;
    const [factsRes, overrides] = await Promise.all([
        env.DB.prepare(`SELECT id, predicate, value_text, value_number, value_json, value_entity_id,
              source_kind, source, confidence, verified_score, observed_at,
              is_current, superseded_by_override
         FROM facts
        WHERE entity_id = ?${factWhere}
        ORDER BY observed_at DESC LIMIT ?`).bind(entityId, limit).all(),
        loadCurrentOverrides(env, entityId),
    ]);
    const out = [];
    const overridePredsSeen = new Set();
...
export async function insertFactsBatch(env, entityId, patches, source, sourceKind = "scrape", evidenceUrl = null) {
    let n = 0;
    for (const p of patches) {
        if (p.value_text == null && p.value_number == null && p.value_json == null && p.value_entity_id == null)
            continue;
