...
export async function createEntity(env, init) {
    ...
    if (verdict.is_garbage) {
        ...
        void logDataQuality(env, "rejected:" + (init.display_name ?? "<empty>").slice(0, 100), "pre_insert_rejected", verdict.reasons, "pre_insert_guard", null).catch(() => undefined);
        return null;
    }
    ...
    let effectiveKind = init.kind;
    let reclassifiedOrgRole = null;
    if (init.kind === "person") {
    ...
    await env.DB.prepare(`INSERT INTO u_entities (
     ...
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)`).bind(id, effectiveKind, init.display_name ?? null, init.primary_url ?? null, init.primary_domain ?? null, init.primary_email_key ?? null, init.primary_linkedin_key ?? null, init.primary_twitter_handle ?? null, init.primary_github_handle ?? null, now, now).run();
    await env.DB.prepare(`INSERT INTO entity_history (id, entity_id, action, source, changed_at) VALUES (?, ?, 'create', 'system', ?)`).bind(crypto.randomUUID(), id, now).run();
    ...
    if (reclassifiedOrgRole) {
        ...
        void logDataQuality(env, id, "reclassified", [`org_role:${reclassifiedOrgRole}`, "reclassified_on_write"], "pre_insert_guard", null).catch(() => undefined);
    }
    ...
    if (effectiveKind === "org" && (init.primary_url || init.primary_domain) && !init.suppressAutoProfileFill) {
        const wf = env.WF_PROFILE_FILLER;
        if (wf) {
            try {
    ...
    return (await env.DB.prepare(`SELECT * FROM u_entities WHERE id = ?`).bind(id).first());
}
export async function addRole(env, entityId, role, opts) {
    try {
       ...
       ON CONFLICT(entity_id, role) DO UPDATE SET
         ...
         confidence = MAX(confidence, excluded.confidence)`).bind(entityId, role, opts?.is_primary ? 1 : 0, opts?.source ?? null, opts?.confidence ?? 1).run();
    }
    catch (e) {
        console.warn("addRole failed", role, e.message);
    }
}
export async function getLegacyEntityId(env, table, legacyId) {
    const r = await env.DB.prepare(`SELECT entity_id FROM entity_legacy_map WHERE legacy_table = ? AND legacy_id = ?`).bind(table, String(legacyId)).first();
    return r?.entity_id ?? null;
}
export async function setLegacyEntityId(env, table, legacyId, entityId) {
    try {
        await env.DB.prepare(`INSERT INTO entity_legacy_map (legacy_table, legacy_id, entity_id)
       VALUES (?, ?, ?) ON CONFLICT DO NOTHING`).bind(table, String(legacyId), entityId).run();
    }
    catch (e) {
        console.warn("setLegacyEntityId failed", table, legacyId, e.message);
    }
}
