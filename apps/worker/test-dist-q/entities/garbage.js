...
const LEADER_RE = /^(introducing|announcing|welcome to|how|why|what|when|where|the future of|inside)\s+/i;
 ...
 */
function looksDomainDerived(name, domain) {
    if (!domain)
        return false;
    const label = domain.toLowerCase().replace(/^www\./, "").split(".")[0] ?? "";
    if (label.length < SLUG_NAME_MIN_LEN)
        return false;
...
function orgRoleForTokens(tokensLower) {
    const set = new Set(tokensLower);
    if (set.has("accelerator") || set.has("incubator"))
        return "accelerator";
    if (set.has("fund") || set.has("funds"))
        return "fund";
 ...
 */
export function classifyPersonName(rawName) {
    const raw = (rawName ?? "").trim();
    if (!raw)
        return { verdict: "junk", reasons: ["empty_name"] };
    const lower = raw.toLowerCase();
    const tokens = raw.split(/\s+/).filter(Boolean);
    const norm = tokens.map(normToken).filter(Boolean);
    // 1. Exact known-junk phrase / place name.
    if (KNOWN_JUNK_PHRASES.has(lower))
        return { verdict: "junk", reasons: ["known_junk_phrase"] };
    if (PLACE_NAMES.has(lower))
        return { verdict: "junk", reasons: ["place_name"] };
    ...
    const cleanTwoToken = tokens.length === 2 && tokens.every((t) => /^[\p{Lu}][\p{L}'’.\-]*$/u.test(t));
    if (!cleanTwoToken) {
        for (const t of norm) {
            if (HARD_JUNK_WORDS.has(t))
                return { verdict: "junk", reasons: [`junk_word:${t}`] };
        }
    }
    ...
    if (hasLegal || lastIsOrgSuffix) {
        ...
        const distinctive = norm.filter((t, i) => !GENERIC_WORDS.has(t) &&
...
export function isGarbage(input) {
    const reasons = [];
    ...
    if (NO_ALNUM_RE.test(raw))
        reasons.push("no_alphanumeric_chars");
    // Rule 4: page-title with `|` brand fragment.
    if (PIPE_TITLE_RE.test(raw))
        reasons.push("page_title_pipe_fragment");
    // Rule 5: blog/press leader phrase.
    if (LEADER_RE.test(raw))
        reasons.push("press_leader_phrase");
    // Rule 6: known UI / nav string (case-insensitive exact match).
    if (KNOWN_UI_STRINGS.has(raw.toLowerCase()))
        reasons.push("known_ui_string");
    // Rule 6c: listicle / directory page title captured as an entity name.
    if (LISTICLE_RES.some((re) => re.test(raw)))
        reasons.push("listicle_page_title");
    // Rule 6d: name is just the domain slug — the crawler never extracted a
    // real name and fell back to the URL.
    if (looksDomainDerived(raw, input.primary_domain))
        reasons.push("domain_slug_name");
    ...
    if (/&(amp|lt|gt|quot|#x?[0-9a-f]+);/i.test(raw))
        reasons.push("literal_html_entity");
    ...
    if (input.kind === "person") {
        if (!/\s/.test(raw))
            reasons.push("person_no_space");
        if (/[|/:]/.test(raw))
            reasons.push("person_contains_separator");
        ...
        if (cls.verdict === "junk") {
            for (const code of cls.reasons)
                reasons.push(`name_${code}`);
        }
    }
...
// ---------------------------------------------------------------------------
export async function isStructurallyOrphan(env, entityId, options = {}) {
    ...
    try {
        const row = await env.DB.prepare(`SELECT
         ...
         (SELECT (julianday('now') - julianday(created_at)) * 24 FROM u_entities WHERE id = ?1) AS age_hours`).bind(entityId).first();
        if (!row)
        ...
        console.warn("isStructurallyOrphan probe failed", entityId, e.message);
    }
    return { orphan: false, reasons };
}
...
export async function aiSecondOpinion(env, input) {
    ...
    try {
        ...
        if (!match)
            return { verdict: "uncertain", confidence: 0, reason: "ai_no_json" };
        ...
        return { verdict: "uncertain", confidence: 0, reason: "ai_error:" + e.message };
    }
}
/**
 ...
 */
export async function evaluateEntity(env, input, opts = {}) {
...
// ---------------------------------------------------------------------------
export async function logDataQuality(env, entityId, issue, reasons, source, actorEmail, 
...
payload) {
    try {
        await env.DB.prepare(`INSERT INTO data_quality_log (entity_id, issue, reasons_json, source, actor_email)
       VALUES (?, ?, ?, ?, ?)`).bind(entityId, issue, JSON.stringify(payload ?? reasons), source, actorEmail ?? null).run();
    }
    catch (e) {
        console.warn("data_quality_log insert failed", entityId, e.message);
    }
}
export async function softDeleteEntity(env, entityId, reasons, source, actorEmail) {
    await env.DB.prepare(`UPDATE u_entities
        SET status = 'soft_deleted',
            deleted_reason = COALESCE(deleted_reason, ?),
            updated_at = datetime('now')
      WHERE id = ? AND status != 'soft_deleted'`).bind("garbage_detector_v1:" + reasons.join(","), entityId).run();
    try {
        ...
        const roles = await env.DB.prepare(`SELECT role, is_primary, source, confidence FROM entity_roles WHERE entity_id = ?`).bind(entityId).all();
        ...
        await logDataQuality(env, entityId, "soft_deleted_roles", [], source, actorEmail, roles.results ?? []);
        await env.DB.prepare(`DELETE FROM entity_roles WHERE entity_id = ?`).bind(entityId).run();
    }
    catch (e) {
        console.warn("entity_roles delete during soft-delete failed", entityId, e.message);
    }
    await logDataQuality(env, entityId, "soft_deleted", reasons, source, actorEmail);
}
export async function restoreEntity(env, entityId, actorEmail) {
    await env.DB.prepare(`UPDATE u_entities
        SET status = 'active',
            deleted_reason = NULL,
            updated_at = datetime('now')
      WHERE id = ?`).bind(entityId).run();
    ...
    let rolesRestored = 0;
    try {
        const parked = await env.DB.prepare(`SELECT reasons_json FROM data_quality_log
        WHERE entity_id = ? AND issue = 'soft_deleted_roles'
        ORDER BY detected_at DESC, id DESC LIMIT 1`).bind(entityId).first();
        const rows = parked?.reasons_json ? JSON.parse(parked.reasons_json) : [];
        for (const r of Array.isArray(rows) ? rows : []) {
            if (!r || typeof r.role !== "string" || !r.role)
                continue;
            await env.DB.prepare(`INSERT INTO entity_roles (entity_id, role, is_primary, source, confidence)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(entity_id, role) DO NOTHING`).bind(entityId, r.role, r.is_primary ?? 0, r.source ?? "restore", r.confidence ?? 0.5).run();
            rolesRestored += 1;
        }
    }
    catch (e) {
        ...
        await logDataQuality(env, entityId, "restore_roles_failed", [e.message], "operator", actorEmail);
    }
    await logDataQuality(env, entityId, "restored", [`roles_restored:${rolesRestored}`], "operator", actorEmail);
}
export async function purgeEntity(env, entityId, actorEmail) {
    ...
    for (const sql of cascades) {
        try {
            ...
            console.warn("purge cascade failed", sql.slice(0, 40), e.message);
        }
    }
...
// ---------------------------------------------------------------------------
export async function reclassifyPersonAsOrg(env, entity, orgRole, reasons, source, actorEmail) {
    // 1. Flip kind in place — removes it from the People list immediately.
    await env.DB.prepare(`UPDATE u_entities SET kind = 'org', updated_at = datetime('now') WHERE id = ?`).bind(entity.id).run();
    ...
    let priorRoles = [];
    try {
        const existing = await env.DB.prepare(`SELECT role FROM entity_roles WHERE entity_id = ?`).bind(entity.id).all();
        priorRoles = (existing.results ?? []).map((x) => x.role);
        await env.DB.prepare(`DELETE FROM entity_roles WHERE entity_id = ?`).bind(entity.id).run();
        await env.DB.prepare(`INSERT INTO entity_roles (entity_id, role, is_primary, source, confidence)
       VALUES (?, ?, 1, ?, 1)
       ON CONFLICT(entity_id, role) DO UPDATE SET is_primary = 1`).bind(entity.id, orgRole, source).run();
    }
    catch (e) {
        console.warn("reclassify role swap failed", entity.id, e.message);
    }
    ...
    if (entity.display_name && (entity.primary_domain || entity.primary_url)) {
        try {
            ...
            console.warn("reclassify upsertFirm failed", entity.id, e.message);
        }
    }
    await logDataQuality(env, entity.id, "reclassified", [...reasons, `org_role:${orgRole}`,
        priorRoles.length ? `prior_roles:${priorRoles.join(",")}` : "prior_roles:none",
        firmListed ? "firm_listed" : "firm_row_skipped_no_domain"], source, actorEmail);
    return { reclassified: true, firm_listed: firmListed };
}
export async function runCleanupSweep(env, opts = {}) {
    ...
    const rows = await env.DB.prepare(`SELECT id, kind, display_name, primary_url, primary_domain, primary_email_key, primary_linkedin_key
       FROM u_entities
      WHERE ${where}
    ...
    for (const r of items) {
        ...
        if (r.kind === "person") {
            ...
            if (cls.verdict === "organization" && cls.orgRole) {
                ...
                if (personalLinkedin || hasEmail) {
                    for (const code of cls.reasons)
                        byReason[`review_${code}`] = (byReason[`review_${code}`] ?? 0) + 1;
                    await logDataQuality(env, r.id, "needs_review", [...cls.reasons, "org_name_with_person_signal"], source, opts.actorEmail ?? null);
                    needsReview += 1;
                    continue;
                }
                try {
                    await reclassifyPersonAsOrg(env, { id: r.id, display_name: r.display_name, primary_url: r.primary_url, primary_domain: r.primary_domain }, cls.orgRole, cls.reasons, source, opts.actorEmail ?? null);
                    reclassified += 1;
                    byReason[`reclassified_${cls.orgRole}`] = (byReason[`reclassified_${cls.orgRole}`] ?? 0) + 1;
                }
                catch (e) {
                    console.warn("sweep reclassify failed", r.id, e.message);
                }
                continue;
            }
        }
        ...
        try {
            ...
            console.warn("sweep soft-delete failed", r.id, e.message);
        }
    }
    const result = {
