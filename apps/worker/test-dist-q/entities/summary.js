...
import { getEffectiveFacts } from "./facts";
const SOURCE_PRIORITY = {
    manual: 5, enrichment: 4, import: 3, scrape: 2, ai: 1, inferred: 0,
};
function pickBestFact(rows) {
    if (!rows.length)
        return null;
...
export async function rebuildSummary(env, entityId) {
    ...
    // Task #3 (Editable Profiles): the summary input is the EFFECTIVE
    // facts view — overrides win, overridden_attempt rows are filtered.
    // Same resolver as the per-entity read path in query.ts, so the two
    // call sites cannot drift.
    const [effective, tagsRes, rolesRes, channelsRes] = await Promise.all([
        getEffectiveFacts(env, entityId),
        env.DB.prepare(`SELECT taxonomy, slug, weight FROM entity_tags WHERE entity_id = ?`).bind(entityId).all(),
        env.DB.prepare(`SELECT role, is_primary, confidence FROM entity_roles WHERE entity_id = ?`).bind(entityId).all(),
    ...
    const facts = effective
        .filter((e) => !e.overridden_attempt)
        .map((e) => ({
        predicate: e.predicate,
        value_text: e.value_text,
        value_number: e.value_number,
        value_json: e.value_json != null ? (typeof e.value_json === "string" ? e.value_json : JSON.stringify(e.value_json)) : null,
    ...
    let employerDisplay = null;
    if (employerEntityId) {
        const r = await env.DB.prepare(`SELECT display_name FROM u_entities WHERE id = ?`).bind(employerEntityId).first();
    ...
    await env.DB.prepare(`INSERT INTO entity_summary (
