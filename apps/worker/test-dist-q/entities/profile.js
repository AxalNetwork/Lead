...
import { EMITTED_PREDICATES, PREDICATE_MAP, } from "./profile-predicates";
...
async function withProfileLock(env, entityId, fn) {
    if (!env.ENTITY_LOCK)
        return await fn();
...
async function mirrorFact(env, args) {
    ...
    try {
        await env.DB.prepare(`INSERT INTO facts (
       ...
       ) VALUES (?, ?, ?, ?, ?, ?, NULL, 'enrichment', ?, ?, ?, ?, NULL, NULL, 1, ?)`).bind(crypto.randomUUID(), args.entityId, args.predicate, valueText, valueNumber, valueJsonStr, args.sourceUrl, args.sourceUrl, confidence, now, hash).run();
    }
    catch (e) {
        const msg = e.message || "";
        if (/UNIQUE/i.test(msg)) {
            await env.DB.prepare(`UPDATE facts
            SET value_text = ?, value_number = ?, value_json = ?,
                confidence = MAX(confidence, ?),
                observed_at = ?, is_current = 1
          WHERE hash = ?`).bind(valueText, valueNumber, valueJsonStr, confidence, now, hash).run();
        }
        else {
            throw e;
        }
    ...
    try {
        await enqueueSummaryRebuild(env, args.entityId);
    }
    catch { /* best-effort */ }
}
function requireSourceUrl(helper, url) {
    if (!url || typeof url !== "string" || url.trim().length === 0) {
        throw new Error(`profile.${helper}: source_url is required (public-signal-only constraint)`);
    }
    return url;
}
function requireNonEmpty(helper, field, v) {
...
export async function setPersonIdentity(env, input) {
    ...
    await withProfileLock(env, input.entityId, async () => {
       ...
       ON CONFLICT(entity_id) DO UPDATE SET
         ...
         updated_at       = excluded.updated_at`).bind(input.entityId, input.fullName ?? null, input.preferredName ?? null, input.pronouns ? JSON.stringify(input.pronouns) : null, input.birthYear ?? null, input.nationality ?? null, input.languages ? JSON.stringify(input.languages) : null, input.timezone ?? null, input.locationCity ?? null, input.locationCountry ?? null, input.headshotUrl ?? null, input.sourceUrl ?? null, isOperator ? 1 : 0, input.confidence ?? 1.0, now, now).run();
...
// =========================================================================
export async function addCareerEntry(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO career_history (
       ...
       ON CONFLICT(entity_id, COALESCE(organization_entity_id,''), COALESCE(started_at,'')) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.organizationEntityId ?? null, input.organizationName, input.roleTitle ?? null, input.seniority ?? null, input.department ?? null, input.startedAt ?? null, input.endedAt ?? null, input.isCurrent ? 1 : 0, input.summary ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
...
// =========================================================================
export async function addBoardSeat(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO board_seats (
       ...
       ON CONFLICT(entity_id, organization_name, COALESCE(started_at,'')) DO UPDATE SET
         ...
         observed_at    = excluded.observed_at,
         updated_at     = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.organizationEntityId ?? null, input.organizationName, input.role ?? null, input.isIndependent ? 1 : 0, input.committee ?? null, input.startedAt ?? null, input.endedAt ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
...
// =========================================================================
export async function addEducation(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO education_history (
       ...
       ON CONFLICT(entity_id, institution, COALESCE(degree,''), COALESCE(ended_year,0)) DO UPDATE SET
         ...
         observed_at  = excluded.observed_at,
         updated_at   = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.institution, input.degree ?? null, input.field ?? null, input.startedYear ?? null, input.endedYear ?? null, input.honors ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
...
// =========================================================================
export async function addFamilyTie(env, input) {
    ...
    if (input.isPublic === false && input.isOperatorAsserted !== true) {
        throw new Error("profile.addFamilyTie: private family ties (isPublic=false) require " +
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO family_ties (
       ...
       ON CONFLICT(entity_id, relation_type, related_name) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.relationType, input.relatedName, input.relatedEntityId ?? null, input.notes ?? null, input.isPublic ? 1 : 0, sourceUrl, input.confidence ?? 1.0, now, now).run();
...
// =========================================================================
export async function addPreference(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO person_preferences (
       ...
       ON CONFLICT(entity_id, preference_key) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.preferenceKey, input.valueText ?? null, input.valueJson ? JSON.stringify(input.valueJson) : null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addInterest(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO person_interests (
       ...
       ON CONFLICT(entity_id, interest_category, interest_value) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.interestCategory, input.interestValue, input.weight ?? 1.0, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addLifestyleSignal(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO lifestyle_signals (
       ...
       ON CONFLICT(entity_id, signal_key) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.signalKey, input.valueText ?? null, input.valueJson ? JSON.stringify(input.valueJson) : null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addTravelPattern(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO travel_patterns (
       ...
       ON CONFLICT(entity_id, pattern_kind, place, COALESCE(starts_at,'')) DO UPDATE SET
         ...
         observed_at  = excluded.observed_at,
         updated_at   = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.patternKind, input.place, input.countryIso2 ?? null, input.startsAt ?? null, input.endsAt ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addConferenceAttendance(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO conference_attendance (
       ...
       ON CONFLICT(entity_id, conference_name, year) DO UPDATE SET
         ...
         observed_at   = excluded.observed_at,
         updated_at    = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.conferenceName, input.year, input.role ?? null, input.sessionTopic ?? null, input.city ?? null, input.countryIso2 ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
...
// =========================================================================
export async function addGoal(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO person_goals (
       ...
       ON CONFLICT(entity_id, goal_kind, goal_text) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.goalKind, input.goalText, input.targetDate ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addConversationHook(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO conversation_hooks (
       ...
       ON CONFLICT(entity_id, hook_kind, hook_text) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.hookKind, input.hookText, input.relatedEntityId ?? null, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
...
// =========================================================================
export async function addAppreciationSignal(env, input) {
    ...
    const now = input.observedAt ?? nowIso();
    await withProfileLock(env, input.entityId, async () => {
        await env.DB.prepare(`INSERT INTO appreciation_signals (
       ...
       ON CONFLICT(entity_id, signal_kind, signal_text) DO UPDATE SET
         ...
         observed_at = excluded.observed_at,
         updated_at  = excluded.updated_at`).bind(crypto.randomUUID(), input.entityId, input.signalKind, input.signalText, sourceUrl, input.confidence ?? 1.0, now, now).run();
        await mirrorFact(env, {
            entityId: input.entityId,
            predicate,
            sourceUrl,
