...
function scoreSize(p, emp, band) {
    if (p.size_min == null && p.size_max == null && p.size_bands.length === 0)
        return 50;
    if (band && p.size_bands.length && p.size_bands.includes(band))
        return 100;
    if (emp == null)
        return 25;
...
function scoreGeo(p, iso) {
    if (!p.geos.length)
        return 50;
    if (!iso)
        return 20;
    const i = lc(iso);
    if (p.geos.includes(i))
        return 100;
    // Region bucketing
    const REGION = {
...
function scoreIndustry(p, primary, all) {
    if (!p.industries.length)
        return 50;
    const want = new Set(p.industries.map(lc));
    if (primary && want.has(lc(primary)))
        return 100;
    for (const i of all.map(lc))
        if (want.has(i))
            return 80;
    return 0;
}
function scoreTech(p, techs) {
    const have = new Set(techs.map(lc));
    if (p.techs_excluded.some((t) => have.has(lc(t))))
    ...
    if (!p.techs_preferred.length && !p.techs_required.length)
        return { score: 50, pass: true };
...
function scoreBuyer(p, b) {
    let s = 0;
    let denom = 0;
    if (p.buyer_titles.length) {
        denom += 50;
        const t = lc(b.title);
        if (t && p.buyer_titles.some((x) => t.includes(lc(x))))
            s += 50;
    }
    if (p.buyer_seniority.length) {
        denom += 30;
        if (b.seniority && p.buyer_seniority.includes(lc(b.seniority)))
            s += 30;
    }
    if (p.buyer_departments.length) {
        denom += 20;
        if (b.department && p.buyer_departments.includes(lc(b.department)))
            s += 20;
    }
    if (denom === 0)
        return 50;
    // Decision-maker bonus
    if (b.is_decision_maker)
        s = Math.min(denom, s + 5);
    return Math.round((s / denom) * 100);
}
function scoreBuyersForAccount(p, buyers) {
    if (!buyers.length)
        return p.buyer_titles.length || p.buyer_seniority.length ? 0 : 50;
    let best = 0;
    for (const b of buyers)
        best = Math.max(best, scoreBuyer(p, b));
    return best;
}
export function checkHardFilters(p, account, buyer) {
    const reasons = [];
    const f = p.hard_filters || {};
    const acc = account ?? buyer?.account ?? null;
    if (f.require_domain && acc && !acc.domain) {
        reasons.push("missing_domain");
        return { pass: false, reasons };
    }
    if (Array.isArray(f.statuses_in) && acc && !f.statuses_in.map(lc).includes(lc(acc.status))) {
        reasons.push(`status_not_in:${acc.status}`);
        return { pass: false, reasons };
    }
    if (Array.isArray(f.exclude_country_iso2) && acc && acc.hq_country_iso2 && f.exclude_country_iso2.map(lc).includes(lc(acc.hq_country_iso2))) {
        reasons.push(`country_excluded:${acc.hq_country_iso2}`);
        return { pass: false, reasons };
    }
    if (Array.isArray(f.country_iso2_in) && acc && (!acc.hq_country_iso2 || !f.country_iso2_in.map(lc).includes(lc(acc.hq_country_iso2)))) {
        reasons.push("country_not_in");
        return { pass: false, reasons };
    }
    if (Array.isArray(f.funding_stage_in) && acc && (!acc.funding_stage || !f.funding_stage_in.map(lc).includes(lc(acc.funding_stage)))) {
        reasons.push("funding_stage_not_in");
        return { pass: false, reasons };
    }
    if (f.is_decision_maker && buyer && !buyer.is_decision_maker) {
        reasons.push("not_decision_maker");
        return { pass: false, reasons };
    }
    return { pass: true, reasons };
}
export function recencyBoost(p, lastModifiedISO) {
    // Override wins; otherwise small boost (max 1.15x) for entities updated
    // within the last 7 days. Capped 1.0..1.2 per spec.
    if (p.recency_boost && p.recency_boost > 0)
        return clamp(p.recency_boost, 1.0, 1.2);
    if (!lastModifiedISO)
        return 1.0;
...
export function scoreEntity(p, ctx) {
    const reasons = [];
...
export function buildEmbeddingText(p) {
    ...
    if (p.geos.length)
        parts.push(`Geos: ${p.geos.join(", ")}`);
    if (p.size_min || p.size_max)
        parts.push(`Size: ${p.size_min ?? "?"}–${p.size_max ?? "?"} FTE`);
    if (p.techs_required.length)
        parts.push(`Required tech: ${p.techs_required.join(", ")}`);
    if (p.techs_preferred.length)
        parts.push(`Preferred tech: ${p.techs_preferred.join(", ")}`);
    if (p.signal_kinds.length)
        parts.push(`Watch signals: ${p.signal_kinds.join(", ")}`);
    if (p.buyer_titles.length)
        parts.push(`Buyer titles: ${p.buyer_titles.join(", ")}`);
    if (p.buyer_seniority.length)
        parts.push(`Buyer seniority: ${p.buyer_seniority.join(", ")}`);
    if (p.buyer_departments.length)
        parts.push(`Buyer departments: ${p.buyer_departments.join(", ")}`);
    return parts.join(" | ");
}
