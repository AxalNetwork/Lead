...
export function trackAi(env, args) {
...
export function trackVectorize(env, args) {
    try {
        env.ANALYTICS?.writeDataPoint({
...
async function rollupVectorizeOp(env, op, index) {
    try {
        const day = new Date().toISOString().slice(0, 10);
        await env.DB.prepare(`INSERT INTO ai_cost_daily (day, purpose, model, neurons, cost_usd, calls)
       VALUES (?, ?, ?, 0, 0, 1)
       ON CONFLICT(day, purpose, model) DO UPDATE SET calls = calls + 1`).bind(day, `vectorize_${op}`, index).run();
    }
    catch (e) {
        console.warn("vectorize rollup failed", e.message);
    }
}
export function trackFetch(env, args) {
    try {
        env.ANALYTICS?.writeDataPoint({
            indexes: ["fetch"],
...
async function rollupAiCost(env, purpose, model, neurons, costUsd) {
    try {
        const day = new Date().toISOString().slice(0, 10);
        await env.DB.prepare(`INSERT INTO ai_cost_daily (day, purpose, model, neurons, cost_usd, calls)
       VALUES (?, ?, ?, ?, ?, 1)
       ON CONFLICT(day, purpose, model) DO UPDATE SET
         ...
         calls = calls + 1`).bind(day, purpose, model, neurons, costUsd).run();
    }
    catch (e) {
        console.warn("ai_cost_daily rollup failed", e.message);
    }
}
