...
const SENIORITY_INDEX = Object.fromEntries(SENIORITY_LADDER.map((s, i) => [s, i]));
const STAGE_LADDER = [
    "pre_seed", "seed", "series_a", "series_b", "series_c",
    "series_d", "growth", "late", "public",
];
const STAGE_INDEX = Object.fromEntries(STAGE_LADDER.map((s, i) => [s, i]));
...
// ---------------------------------------------------------------------------
export function scoreSeniority(entity, targets) {
...
export function scoreFunction(entityDept, targets) {
...
export function scoreIndustry(entityIndustries, targets) {
    ...
    for (const ei of entityIndustries) {
        ...
        if (tset.has(e)) {
            best = 1.0;
            bestNote = `industry "${ei}" matches target`;
            break;
        }
        ...
        for (const p of parents) {
            if (tset.has(p)) {
                if (best < 0.7) {
                    best = 0.7;
                    bestNote = `industry "${ei}" ⊂ target "${p}"`;
                }
            }
        }
    }
    if (!bestNote)
        bestNote = `industries [${entityIndustries.join(",")}] don't match targets`;
    return { value: best, weight: DEFAULT_WEIGHTS.industry, reason: bestNote };
}
export function scoreCompanySize(emp, minE, maxE) {
    ...
    if (d <= 0.5)
        return { value: 0.5, weight: DEFAULT_WEIGHTS.company_size, reason: `headcount ${emp} adjacent to target` };
    return { value: 0, weight: DEFAULT_WEIGHTS.company_size, reason: `headcount ${emp} outside target [${lo}, ${maxE ?? "∞"}]` };
}
export function scoreStage(entityStages, targets) {
    ...
    for (const e of entityStages) {
        ...
        for (const t of targets) {
            ...
            if (v > best) {
                best = v;
                note = d === 0 ? `stage ${e} matches target` : `stage ${e} adjacent to ${t}`;
            }
        }
    }
...
export function scoreGeo(input) {
...
export function buildRationale(personaName, entityName, employerName, components, score) {
...
export function extractTargets(row) {
    const hard = objFromJson(row.hard_filters_json);
    const stagesFromHard = Array.isArray(hard.stages) ? hard.stages.filter((x) => typeof x === "string")
        : Array.isArray(hard.target_stage) ? hard.target_stage.filter((x) => typeof x === "string")
            : [];
    const center = hard.geo_center && typeof hard.geo_center === "object" ? hard.geo_center : {};
