...
const RELEVANT_PREDICATES = new Set([
...
export function isRelevantPredicate(predicate) {
    if (!predicate)
        return false;
    return RELEVANT_PREDICATES.has(predicate);
}
const DEBOUNCE_SECONDS = 300; // 5 minutes
export async function triggerEntityMatchRefresh(env, entityId) {
    if (!entityId)
        return;
    ...
    try {
        ...
        console.warn("triggerEntityMatchRefresh debounce check failed", entityId, e.message);
    ...
    try {
        ...
        console.warn("triggerEntityMatchRefresh dispatch failed", entityId, e.message);
    }
}
