...
import { DEFAULT_WEIGHTS, MODEL_VERSION, cosine, aggregate, buildRationale, extractTargets, scoreSeniority, scoreFunction, scoreIndustry, scoreCompanySize, scoreStage, scoreGeo, } from "./personaMatchingScorers";
...
export function buildStructuralFallback(reason) {
    const components = {};
    for (const key of Object.keys(DEFAULT_WEIGHTS)) {
        components[key] = { value: 0, weight: 0, reason: "n/a (structural fallback)" };
    }
...
// Entity loader.
// ---------------------------------------------------------------------------
async function loadEmployerFacts(env, employerId) {
    const sum = await env.DB.prepare(`SELECT display_name, country_iso2, sectors_csv, stages_csv FROM entity_summary WHERE entity_id = ?`).bind(employerId).first();
    ...
    const hc = await env.DB.prepare(`SELECT value_number FROM facts WHERE entity_id = ? AND is_current = 1 AND predicate IN ('employees','org.headcount','org.employees','company.employees','company.headcount') AND value_number IS NOT NULL ORDER BY observed_at DESC LIMIT 1`).bind(employerId).first();
    if (!sum && !hc)
        return null;
    return {
...
async function loadEntityCoords(env, entityId) {
    const r = await env.DB.prepare(`SELECT predicate, value_number FROM facts
      WHERE entity_id = ? AND is_current = 1
        ...
        AND value_number IS NOT NULL`).bind(entityId).all();
    let lat = null;
    let lng = null;
    for (const row of r.results ?? []) {
        if (lat == null && (row.predicate.endsWith(".lat") || row.predicate === "geo.lat"))
            lat = row.value_number;
        if (lng == null && (row.predicate.endsWith(".lng") || row.predicate === "geo.lng"))
            lng = row.value_number;
    }
    return { lat, lng };
}
export async function loadPersonEntity(env, entityId) {
    const ent = await env.DB.prepare(`SELECT id, display_name, kind, status FROM u_entities WHERE id = ?`).bind(entityId).first();
    if (!ent)
        return null;
    if (ent.kind !== "person")
        return null;
    if (ent.status === "merged" || ent.status === "soft_deleted")
        return null;
    const sum = await env.DB.prepare(`SELECT country_iso2, region FROM entity_summary WHERE entity_id = ?`).bind(entityId).first();
    const career = await env.DB.prepare(`SELECT role_title, seniority, department, organization_entity_id, organization_name
       FROM career_history
      WHERE entity_id = ?
    ...
    if (!title) {
        const tf = await env.DB.prepare(`SELECT value_text FROM facts WHERE entity_id = ? AND is_current = 1 AND predicate IN ('person.title','title') AND value_text IS NOT NULL ORDER BY observed_at DESC LIMIT 1`).bind(entityId).first();
        title = tf?.value_text ?? null;
    }
    let employer = null;
    if (career?.organization_entity_id) {
        try {
            employer = await loadEmployerFacts(env, career.organization_entity_id);
        }
        catch { /* ignore */ }
    }
...
async function ensureTitleCacheTables(env) {
    try {
        await env.DB.prepare(`CREATE TABLE IF NOT EXISTS persona_title_embeddings (persona_id TEXT PRIMARY KEY, content_hash TEXT NOT NULL, vector_json TEXT NOT NULL, model TEXT NOT NULL DEFAULT 'bge-base-en-v1.5', updated_at TEXT NOT NULL DEFAULT (datetime('now')))`).run();
        await env.DB.prepare(`CREATE TABLE IF NOT EXISTS entity_title_embeddings (entity_id TEXT PRIMARY KEY, content_hash TEXT NOT NULL, vector_json TEXT NOT NULL, model TEXT NOT NULL DEFAULT 'bge-base-en-v1.5', updated_at TEXT NOT NULL DEFAULT (datetime('now')))`).run();
    }
    catch { /* best-effort */ }
}
async function getOrEmbedTitle(env, scope, id, text) {
    ...
    try {
        const row = await env.DB.prepare(`SELECT vector_json FROM ${table} WHERE ${idCol} = ? AND content_hash = ?`).bind(id, hash).first();
        if (row?.vector_json) {
            try {
    ...
    if (vec && vec.length) {
        try {
            await env.DB.prepare(`INSERT INTO ${table} (${idCol}, content_hash, vector_json, updated_at) VALUES (?, ?, ?, datetime('now'))
         ON CONFLICT(${idCol}) DO UPDATE SET content_hash=excluded.content_hash, vector_json=excluded.vector_json, updated_at=excluded.updated_at`).bind(id, hash, JSON.stringify(vec)).run();
        }
        catch (e) {
            console.warn("title embedding cache write failed", scope, id, e.message);
        }
    }
    return vec;
}
async function titleSimilarity(env, personaId, personaText, entityId, entityTitle) {
    ...
    try {
        ...
        return { value: 0, weight: DEFAULT_WEIGHTS.title_sim, reason: `embed error: ${e.message.slice(0, 60)}` };
    }
}
// ---------------------------------------------------------------------------
// Public API.
// ---------------------------------------------------------------------------
export async function scoreEntityForPersona(env, persona, entity) {
    ...
    const components = { title_sim, seniority, function: fnc, industry, company_size, stage, geo };
...
export async function scoreEntity(env, personaId, entityId) {
...
async function ensureJobsTable(env) {
    try {
        await env.DB.prepare(`CREATE TABLE IF NOT EXISTS persona_match_jobs (
         id TEXT PRIMARY KEY,
         kind TEXT NOT NULL,
...
export async function recordMatchJob(env, kind, status, details) {
    await ensureJobsTable(env);
    try {
        await env.DB.prepare(`INSERT INTO persona_match_jobs (id, kind, status, persona_id, entity_id, details_json)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), kind, status, details.personaId ?? null, details.entityId ?? null, JSON.stringify(details)).run();
    }
    catch (e) {
        console.warn("recordMatchJob failed", kind, status, e.message);
    }
}
async function isCancelled(env, jobId) {
    if (!jobId)
        return false;
    try {
        const r = await env.DB.prepare("SELECT status FROM jobs WHERE id = ?").bind(jobId).first();
...
export async function upsertMatch(env, personaId, entityId, result, opts = {}) {
    const source = opts.source ?? "auto";
    const evidence = JSON.stringify({
        components: Object.fromEntries(Object.keys(result.components).map((k) => [k, {
    ...
    if (source === "auto") {
        await env.DB.prepare(`INSERT INTO persona_entity_matches (persona_id, entity_id, score, match_evidence_json, source, last_scored_at, model_version)
       ...
       ON CONFLICT(persona_id, entity_id) DO UPDATE SET
         ...
         model_version = CASE WHEN persona_entity_matches.source = 'manual' THEN persona_entity_matches.model_version ELSE excluded.model_version END`).bind(personaId, entityId, result.score, evidence, MODEL_VERSION).run();
        return;
    }
    await env.DB.prepare(`INSERT INTO persona_entity_matches (persona_id, entity_id, score, match_evidence_json, source, last_scored_at, model_version)
     ...
     ON CONFLICT(persona_id, entity_id) DO UPDATE SET
       ...
       source = 'manual',
       last_scored_at = excluded.last_scored_at,
       model_version = excluded.model_version`).bind(personaId, entityId, result.score, evidence, MODEL_VERSION).run();
}
export async function scoreEntityAcrossPersonas(env, entityId, opts = {}) {
    const entity = await loadPersonEntity(env, entityId);
    if (!entity)
        return { scored: 0, errors: 0, halted: false };
    const r = await env.DB.prepare(`SELECT * FROM personas WHERE deleted_at IS NULL AND status = 'active'`).all();
    let scored = 0;
    let errors = 0;
    let halted = false;
    for (const p of r.results ?? []) {
        ...
        try {
            ...
            errors += 1;
            console.warn("scoreEntityAcrossPersonas item failed", p.id, entityId, e.message);
        }
    }
    return { scored, errors, halted };
}
export async function scoreBatch(env, personaId, opts = {}) {
    const persona = await getPersona(env, personaId);
    if (!persona)
        return { scored: 0, errors: 0, pages: 0, halted: false };
    ...
    let halted = false;
    for (;;) {
        if (maxEntities != null && scored + errors >= maxEntities)
            break;
        ...
        const r = await env.DB.prepare(filter.sql).bind(...filter.binds).all();
        ...
        for (const id of ids) {
            try {
                ...
                if (!res)
                    res = buildStructuralFallback(plugin.explainMatch(id));
                await upsertMatch(env, personaId, id, res);
                scored += 1;
            }
            catch (e) {
                errors += 1;
                console.warn("scoreBatch item failed", personaId, id, e.message);
            }
        }
...
export async function refreshStaleMatches(env, opts = {}) {
    const staleDays = Math.max(1, opts.staleDays ?? 30);
    const limit = Math.min(Math.max(1, opts.limit ?? 500), 5000);
    const r = await env.DB.prepare(`SELECT persona_id, entity_id FROM persona_entity_matches
      WHERE source = 'auto' AND datetime(last_scored_at) < datetime('now', ?)
      ORDER BY last_scored_at ASC LIMIT ?`).bind(`-${staleDays} days`, limit).all();
    let refreshed = 0;
    let errors = 0;
    let halted = false;
    for (const row of r.results ?? []) {
        ...
        try {
            ...
            errors += 1;
            console.warn("refreshStaleMatches item failed", row.persona_id, row.entity_id, e.message);
        }
    }
    return { refreshed, errors, halted };
}
export async function listCandidates(env, personaId, opts) {
    const minScore = Math.max(0, Math.min(1, opts.minScore ?? 0));
    const limit = Math.min(Math.max(1, opts.limit ?? 50), 500);
    const offset = Math.max(0, opts.offset ?? 0);
    const r = await env.DB.prepare(`SELECT pem.persona_id, pem.entity_id, pem.score, pem.source, pem.last_scored_at, pem.model_version, pem.match_evidence_json,
      ...
      LIMIT ? OFFSET ?`).bind(personaId, minScore, limit, offset).all();
    return (r.results ?? []).map((row) => {
        let components = {};
        let rationale = "";
        if (row.match_evidence_json) {
            try {
                const j = JSON.parse(row.match_evidence_json);
                if (j.components)
