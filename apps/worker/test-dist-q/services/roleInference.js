...
async function loadKnownInvestorDomains(env) {
    if (DOMAIN_CACHE && Date.now() - DOMAIN_CACHE.at < DOMAIN_CACHE_TTL_MS)
        return DOMAIN_CACHE.set;
    const set = new Set(VC_DOMAINS_STATIC);
    try {
        const r = await env.DB.prepare("SELECT domain FROM known_investor_domains").all();
...
export async function inferAndAssignRoles(env, entityId, ctx) {
    const roles = [];
    ...
    if (ctx.importLabel && /investor/i.test(ctx.importLabel)) {
        if (ctx.kind === "person")
            roles.push({ role: "investor", confidence: 0.95 });
        if (ctx.kind === "org")
            roles.push({ role: "investor_firm", confidence: 0.95 });
    }
    if (domain) {
        ...
        if (known.has(domain)) {
            if (ctx.kind === "org")
                roles.push({ role: "investor_firm", confidence: 0.95 });
            if (ctx.kind === "person")
                roles.push({ role: "investor", confidence: 0.9 });
        }
    }
    if (hasAny(text, ["portfolio companies", "we invest in", "our fund", "aum", "check size"])) {
        if (ctx.kind === "org")
            roles.push({ role: "investor_firm", confidence: 0.85 });
        if (ctx.kind === "person")
    ...
    for (const role of roles) {
        ...
        try {
            await env.DB.prepare(`INSERT OR IGNORE INTO entity_roles (entity_id, role, is_primary, source, confidence)
         VALUES (?, ?, ?, ?, ?)`).bind(entityId, role.role, role.role === "lead" || role.role === "investor_firm" || role.role === "investor" ? 1 : 0, src, role.confidence).run();
            await env.DB.prepare(`UPDATE entity_roles
            SET confidence = MAX(confidence, ?),
                source     = ?
          WHERE entity_id = ? AND role = ?
            AND source LIKE 'role_inference:%'`).bind(role.confidence, src, entityId, role.role).run();
        }
        catch (e) {
            console.warn("inferAndAssignRoles addRole failed", role.role, e.message);
        }
    }
}
