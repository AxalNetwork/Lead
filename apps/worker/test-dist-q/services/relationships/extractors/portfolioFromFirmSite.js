...
export async function extract(env, opts = {}) {
    ...
    const rows = await safeAll(env, `SELECT fp.firm_id, m.entity_id AS firm_entity_id, fp.company_name, fp.company_domain,
    ...
    for (const r of rows) {
        ...
        let company = null;
        if (r.company_domain)
            company = await resolveEntityId(env, r.company_domain, "org");
        if (!company)
            company = await resolveEntityId(env, r.company_name, "org");
        if (!company) {
            unresolved += 1;
            continue;
        }
