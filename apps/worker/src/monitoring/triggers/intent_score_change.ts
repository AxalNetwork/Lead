...
export const evalIntentScoreChange: EvaluatorFn = async (ctx) => {
  const change = ctx.diff.find((d) => d.field === "intent_score");
  ...
  return {
    ...
    title: `${ctx.newSummary.display_name ?? ctx.entityId}: intent score ${oldV} → ${newV}`,
    body: `Intent score moved ${oldV} → ${newV} (Δ ${Math.round(newV - oldV)}).`,
    diff: [change],
