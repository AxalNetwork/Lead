// Crypto helpers for OSINT. All pure-TS / WebCrypto; no native deps.
...
// Tiny MD5 implementation — Workers don't expose MD5 via WebCrypto.
// Adapted from a public-domain reference; correct over UTF-8 ASCII inputs
// which is all we need for Gravatar (lowercase trimmed email).
export function md5Hex(input: string): string {
  // From Joseph Myers' md5.js (public domain), slimmed.
  ...
  function md51(s: string): number[] {
    ...
    const s2 = s.substring(i - 64);
    for (let j = 0; j < s2.length; j++) tail[j >> 2] |= s2.charCodeAt(j) << ((j % 4) << 3);
    tail[s2.length >> 2] |= 0x80 << ((s2.length % 4) << 3);
    if (s2.length > 55) { md5cycle(state, tail); for (let k = 0; k < 16; k++) tail[k] = 0; }
    tail[14] = n * 8;
    md5cycle(state, tail);
    return state;
  }
  ...
  // Workers encode input as UTF-8 first.
  const utf8 = unescape(encodeURIComponent(input));
  return hex(md51(utf8));
}

// Hamming distance between two equal-length hex strings (bit count of XOR).
