...
const TABLE: Record<EdgeKind, Record<string, number>> = {
...
export function baselineQuality(kind: EdgeKind, source: string | null | undefined): number {
  const row = TABLE[kind];
  if (!row) return 0.5;
  const src = (source ?? "").trim();
  if (src && row[src] != null) return row[src];
  // Try a prefix match (e.g. "sec.form4.foo" → "sec.form4").
  for (const key of Object.keys(row)) {
    if (key !== "*" && src.startsWith(key)) return row[key];
  }
  return row["*"] ?? 0.5;
}
