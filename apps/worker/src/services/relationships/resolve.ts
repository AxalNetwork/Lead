...
import type { EntityKind } from "../../entities/model";
import { resolveSecEntity } from "../secEdgar/xref";

const cache = new Map<string, string | null>();

function cacheKey(kind: EntityKind, name: string): string {
  ...
  if (!id && /\./.test(trimmed) && !trimmed.includes(" ")) {
    try {
      const r = await env.DB.prepare(
        `SELECT id FROM u_entities WHERE primary_domain = ? AND kind = ? LIMIT 1`,
      ).bind(trimmed.toLowerCase(), kind).first<{ id: string }>();
      id = r?.id ?? null;
    } catch { /* swallow */ }
  }
  cache.set(key, id);
