...
import type { ExtractOpts, OrchestratorSummary, ExtractorRun, ExtractResult } from "./types";
...
interface Extractor {
  NAME: string;
  extract: (env: Env, opts?: ExtractOpts) => Promise<ExtractResult>;
}
...
function emptyRun(): ExtractorRun {
  return { proposed: 0, inserted: 0, merged: 0, unresolved: 0, scanned: 0, errors: 0, error_messages: [] };
}

export async function runAllExtractors(env: Env, opts: ExtractOpts = {}): Promise<OrchestratorSummary> {
  const t0 = Date.now();
  clearResolveCache();
  const summary: OrchestratorSummary = { by_extractor: {}, total_edges: 0, duration_ms: 0 };
  for (const ex of EXTRACTORS) {
    ...
    try {
      ...
      run.errors += 1;
      run.error_messages.push((e as Error).message || "extractor threw");
      console.warn("relationships extractor failed", ex.NAME, (e as Error).message);
    }
 ...
 */
export async function drainInferQueue(env: Env, limit = 200): Promise<{
  drained: number; total_edges: number;
}> {
  let drained = 0, total_edges = 0;
  try {
    const r = await env.DB.prepare(
      `SELECT entity_id FROM relationship_infer_queue ORDER BY queued_at ASC LIMIT ?`,
    ).bind(limit).all<{ entity_id: string }>();
    ...
    for (const id of ids) {
      try {
        ...
        console.warn("drainInferQueue per-entity failed", id, (e as Error).message);
      }
      try {
    ...
    if (!/no such table/i.test(msg)) console.warn("drainInferQueue failed", msg);
  }
 ...
 */
export async function enqueueRelInfer(env: Env, entityId: string, reason: string): Promise<void> {
  if (!entityId) return;
  try {
    if (env.SESSIONS) {
    ...
    await env.DB.prepare(
      `INSERT INTO relationship_infer_queue (entity_id, queued_at, reason)
        VALUES (?, datetime('now'), ?)
       ON CONFLICT(entity_id) DO UPDATE SET queued_at = excluded.queued_at`,
    ).bind(entityId, reason).run();
  } catch (e) {
    const msg = (e as Error).message || "";
    if (!/no such table/i.test(msg)) console.warn("enqueueRelInfer failed", msg);
  }
}
