...
interface ExistingRow {
  id: string;
  backing_fact_ids_json: string | null;
  evidence_count: number | null;
}

function mergeBackingIds(existingJson: string | null, incoming: string[] | undefined): string {
...
export async function persistEdges(env: Env, proposals: EdgeProposal[]): Promise<PersistResult> {
  const res: PersistResult = { inserted: 0, merged: 0, errors: 0, error_messages: [] };
  for (const p of proposals) {
    if (!p.src_entity_id || !p.dst_entity_id || p.src_entity_id === p.dst_entity_id) continue;
    try {
      // Look up existing edge on the unique-index columns. IFNULL on
      // valid_from matches the unique index definition.
      const existing = await env.DB.prepare(
        `SELECT id, backing_fact_ids_json, evidence_count
           FROM rel_edges
          WHERE src_entity_id = ? AND dst_entity_id = ? AND kind = ?
            AND IFNULL(valid_from, '') = IFNULL(?, '')
          LIMIT 1`,
      ).bind(p.src_entity_id, p.dst_entity_id, p.kind, p.valid_from ?? null).first<ExistingRow>();
      ...
      if (existing) {
        ...
        await env.DB.prepare(
          `UPDATE rel_edges
            ...
            WHERE id = ?`,
        ).bind(now, merged, existing.id).run();
        res.merged += 1;
      } else {
        ...
        await env.DB.prepare(
          ...
          id, p.src_entity_id, p.dst_entity_id, p.kind, baseline,
          p.valid_from ?? null, p.valid_to ?? null,
          p.evidence_url ?? null, backing, p.source, now, baseline, now,
        ).run();
        res.inserted += 1;
      }
    } catch (e) {
      ...
      if (/UNIQUE/i.test(msg)) {
        try {
          const again = await env.DB.prepare(
            `SELECT id, backing_fact_ids_json, evidence_count FROM rel_edges
              WHERE src_entity_id = ? AND dst_entity_id = ? AND kind = ?
                AND IFNULL(valid_from, '') = IFNULL(?, '') LIMIT 1`,
          ).bind(p.src_entity_id, p.dst_entity_id, p.kind, p.valid_from ?? null).first<ExistingRow>();
          if (again) {
            ...
            await env.DB.prepare(
              `UPDATE rel_edges
                ...
                WHERE id = ?`,
            ).bind(new Date().toISOString(), merged, again.id).run();
            res.merged += 1;
            continue;
          }
      ...
      res.errors += 1;
      if (res.error_messages.length < 5) res.error_messages.push(msg);
    }
  }
  return res;
}
