...
interface Row {
  id: string; entity_id: string; institution: string;
  started_year: number | null; ended_year: number | null; source_url: string | null;
}
...
export async function extract(env: Env, opts: ExtractOpts = {}): Promise<ExtractResult> {
  ...
  const rows = await safeAll<Row>(
    env,
    `SELECT id, entity_id, institution, started_year, ended_year, source_url
       FROM education_history WHERE ${where} LIMIT ${limit}`,
    ...binds,
  );
