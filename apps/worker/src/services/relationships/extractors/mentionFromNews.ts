...
interface Row { news_item_id: string; entities: string }
...
export async function extract(env: Env, opts: ExtractOpts = {}): Promise<ExtractResult> {
  ...
  if (opts.since) { extra += " AND detected_at >= ?"; binds.push(opts.since); }
  const rows = await safeAll<Row>(
    env,
    `SELECT news_item_id, GROUP_CONCAT(DISTINCT entity_id) AS entities
