...
interface Row {
  id: string; entity_id: string; related_entity_id: string | null;
  related_name: string; notes: string | null; source_url?: string | null;
}
...
export async function extract(env: Env, opts: ExtractOpts = {}): Promise<ExtractResult> {
  ...
  const rows = await safeAll<Row>(
    env,
    `SELECT id, entity_id, related_entity_id, related_name, notes
       FROM family_ties WHERE ${where} LIMIT ${limit}`,
    ...binds,
  );
  ...
  for (const r of rows) {
    ...
    if (!dst) dst = await resolveEntityId(env, r.related_name, "person");
