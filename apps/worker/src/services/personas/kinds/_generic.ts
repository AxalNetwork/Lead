...
import type { MatchResult } from "../../personaMatching";
import { loadPersonEntity, scoreEntityForPersona as scorePersonForPersona } from "../../personaMatching";
import { KINDS, type PersonaKind } from "./taxonomy";

export interface KindCriteriaPlugin {
  kind: PersonaKind;
  /** SQL fragment + binds that selects candidate entity ids from u_entities. */
  defaultEntityFilter(persona: PersonaRow, opts?: { limit?: number; offset?: number }): { sql: string; binds: unknown[] };
  /** Score one entity for this persona. Returns { score, components, rationale }. */
  scoreEntity(env: Env, persona: PersonaRow, entityId: string): Promise<MatchResult | null>;
  /** Human-readable rationale tag. */
  explainMatch(entityId: string): string;
}

export function makeGenericPlugin(kind: PersonaKind): KindCriteriaPlugin {
  ...
  return {
    ...
    defaultEntityFilter(_persona, opts) {
      ...
      const binds: unknown[] = [def.targets];
    ...
    async scoreEntity(env, persona, entityId) {
      ...
      if (def.targets !== "person") return null;
